...
let concept_names_in_class c =
  match c with
    Class.Constructor.IRI i -> (match i.data with IRI.Constructor.IRI s -> s)
  | Class.Constructor.Thing -> ""
  | _ -> failwith "Constructor not implemented."

let rec concept_names_in_class_expression ce =
  match ce.data with
    ClassExpression.Constructor.Class c ->
      [concept_names_in_class c]
  | ClassExpression.Constructor.ObjectIntersectionOf (ce1, ce2) -> 
      List.rev_append (concept_names_in_class_expression ce1) (concept_names_in_class_expression ce2)
  | ClassExpression.Constructor.ObjectSomeValuesFrom (_, e) ->
      concept_names_in_class_expression e
  | _ -> failwith "Constructor not implemented."

let concept_name_in_concept_name_class_expression ce =
  match ce.data with
    ClassExpression.Constructor.Class c -> (match c with
                                             Class.Constructor.IRI i -> (match i.data with
                                                                           IRI.Constructor.IRI s -> s)
                                             | _ -> failwith "concept_name_in_concept_name_class_expression: not a concept name class expression!")
  | _ -> failwith "concept_name_in_concept_name_class_expression: not a concept name class expression!"

let is_concept_name_class c =
  match c with
    Class.Constructor.IRI _ -> true
  | Class.Constructor.Thing -> false
  | _ -> failwith "Constructor not implemented."

let is_concept_name_class_expression ce =
  match ce.data with
    ClassExpression.Constructor.Class c -> is_concept_name_class c
  | ClassExpression.Constructor.ObjectIntersectionOf (_, _) -> false
  | ClassExpression.Constructor.ObjectSomeValuesFrom (_, e) -> false
  | _ -> failwith "Constructor not implemented."

(*
let concept_names_in_and_cascade c =
  match c.data with
    ClassExpression.Constructor.ObjectIntersectionOf(c1, c2) -> 
      match c1.data with 
  | _ -> failwith "Wrong constructor in 'concept_names_in_and_cascade'"
  Cset.fold (fun ce -> fun l -> List.rev_append (concept_names_in_class_expression ce) l) set []
*)

let is_atomic e =
  match e.data with ClassExpression.Constructor.Class _ -> true
                  | _ -> false

let is_domain_exists_expression exp =
  match exp.data with
        ...
        (match rexpf.data with
          ObjectPropertyExpression.Constructor.ObjectProperty rexp ->
            (match (rexp.data, cexp.data) with
              (ObjectProperty.Constructor.IRI i,
               ClassExpression.Constructor.Class cl) ->
               (match i.data with IRI.Constructor.IRI r ->
                 (match cl with 
                    Class.Constructor.Thing ->
                      Some r
                  | _ -> None))
...
let is_range_expression exp =
  match exp.data with
    ClassExpression.Constructor.ObjectSomeValuesFrom (rexpf, cexp)-> 
        (match rexpf.data with
          ObjectPropertyExpression.Constructor.InverseObjectProperty rexp ->
            (match (rexp.data, cexp.data) with
              (ObjectProperty.Constructor.IRI i,
               ClassExpression.Constructor.Class cl) ->
               (match i.data with IRI.Constructor.IRI r ->
...
let handle_normalised_right_hand_side_expression exp topCase conjunctiveCase someTopCase someConceptNameCase =
   match exp.data with
     | ClassExpression.Constructor.Class ce ->
        (match ce with
          Class.Constructor.Thing ->
            topCase ()
        | Class.Constructor.IRI i -> (match i.data with IRI.Constructor.IRI cname -> conjunctiveCase [cname])
        | _ -> failwith "Wrong constructor")
     | ClassExpression.Constructor.ObjectIntersectionOf(c1, c2) -> 
           conjunctiveCase (List.rev_append (concept_names_in_class_expression c1) (concept_names_in_class_expression c2))
     | ClassExpression.Constructor.ObjectSomeValuesFrom (rexpf, cexp)-> 
        (match rexpf.data with
          ObjectPropertyExpression.Constructor.ObjectProperty rexp ->
            (match (rexp.data, cexp.data) with
              (ObjectProperty.Constructor.IRI i,
               ...
               (match i.data with IRI.Constructor.IRI r ->
                 (match cl with 
                    Class.Constructor.Thing -> someTopCase r
                  | Class.Constructor.IRI i2 -> (match i2.data with IRI.Constructor.IRI cname -> someConceptNameCase r cname)
                  | _ -> failwith "Wrong constructor"))
             | _ -> failwith "Wrong constructor")
         | _ -> failwith "Wrong constructor")
     | _ -> failwith "Wrong constructor"
