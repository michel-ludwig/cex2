...
let gc = Gc.get () in
  ...
  gc.Gc.space_overhead <- 92;
  (* A huge stack is needed for efficiency *)
  (* gc.Gc.stack_limit <- 10000000; *)
  Gc.set gc;;
...
let args_spec = ("-d", Arg.Set debug, " Print debug info")::
                ...
                ("--tbx1", Arg.Set_string tbx2FileName, " Specify the first TBox")::
                ("--tbx2", Arg.Set_string tbx1FileName," Specify the second TBox")::
...
let usage_spec = program_info ^
                 "\nUsage: cex25 [-d] [-v] [--mode <concept | instance | conjunctive-query | cq>] " ^
...
let (ont1, occurrenceHash1, leftHandSideSet1, normalisationInformation) = execute_measure_time ("Loading ontology 1 (" ^ !tbx1FileName ^ ")...")
                                                                                               (fun _ -> KRSSParser.load_ontology (open_in !tbx1FileName) (Normalisation.create ())) in

let (ont2, occurrenceHash2, leftHandSideSet2, _) = execute_measure_time ("Loading ontology 2 (" ^ !tbx2FileName ^ ")...")
                                                                        (fun _ -> KRSSParser.load_ontology (open_in !tbx2FileName) normalisationInformation) in
...
let sigma = 
  if string_is_empty !sigmaFileName then
    let computedSigma = match !signature_mode with
                          FromTBox1 -> (get_original_concept_names ont1, get_role_names ont1)
                        | FromTBox2 -> (get_original_concept_names ont2, get_role_names ont2) 
                        | FromIntersection -> (StringSet.intersection (get_original_concept_names ont1) (get_original_concept_names ont2),
                                               StringSet.intersection (get_role_names ont1) (get_role_names ont2))
    in
...
let (cexList, roleDifferencesCount, atomicRHSDiffCount, atomicLHSDiffCount, domainDiffCount, rangeDiffCount, nameMapping) =
  (fun _ -> 
  (* We now add all the role domain and role range bindings to both ontologies for *all* role names occurring *)
  (* in both ontologies. *)
  let allRoleNames = StringSet.union (get_role_names ont1) (get_role_names ont2) in
  let leftHandSideSetWithRoleBindings1 = ref leftHandSideSet1 in
  let leftHandSideSetWithRoleBindings2 = ref leftHandSideSet2 in
  add_domain_range_concept_definitions allRoleNames ont1 occurrenceHash1 leftHandSideSetWithRoleBindings1;
  add_domain_range_concept_definitions allRoleNames ont2 occurrenceHash2 leftHandSideSetWithRoleBindings2;

  assert(OntologyLanguage.is_horn ont1);
  ...
  print_endline ("Renamings used: " ^ (string_of_int (Normalisation.new_symbols_introduced normalisationInformation)));
  ...
  let (roleDifferences, roleDifferencesCount) = RoleDifference.computeRoleDifferences sigma (Ontology.get_RoleDifferenceInformation ont1)
                                                                                            (Ontology.get_RoleDifferenceInformation ont2) in
  ...
  let (atomicRHSCounterExamples, atomicRHSCounterExampleWitnesses, atomicRHSDiffCount) = AtomicRHS.checkCEx sigma nameMapping outputChannel ont1 ont1_index pre_concept_hash1 occurrenceHash1
                                                                                                                                            ont2 ont2_index pre_concept_hash2 occurrenceHash2 in
  print_endline "...done";

  print_endline "Checking whether there exist counter-examples with atomic LHS...";
  let (atomicLHSCounterExamples, atomicLHSCounterExampleWitnesses, atomicLHSDiffCount) = AtomicLHS.checkCEx sigma nameMapping outputChannel ont1 ont1_index pre_concept_hash1 post_concept_hash1 occurrenceHash1 !leftHandSideSetWithRoleBindings1
