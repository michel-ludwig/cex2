  ...
  if not (Hashtbl.mem conceptHash c) then
    ...
    let name = match c with 
              ...
              | _ -> let ren = newRenaming () in
                     ...
                     let owlConcept1 = concept_to_OWL ont1 c in
                     let owlConcept2 = concept_to_OWL ont2 c in
...
let checkCounterExample post_concept_hash1 post_concept_hash2 lhs rhs =
  ...
  if StringSet.mem (Ontology.post_concepts post_concept_hash1 lhsCName) rhsCName then
    begin
    print_endline("For counter-example " ^ (concept_to_string lhs) ^ " " ^ (concept_to_string rhs) ^ ": ");
    print_endline("FAILED: T1 implies this counter-example!");
    result := false;
    end;
  if not (StringSet.mem (Ontology.post_concepts post_concept_hash2 lhsCName) rhsCName) then
    begin
    print_endline("For counter-example " ^ (concept_to_string lhs) ^ " " ^ (concept_to_string rhs) ^ ": ");
    print_endline("FAILED: T2 does not imply this counter-example!");
    result := false;
    end;
  !result

let checkConceptCounterExamples nameMapping ont1 ont2 l =
  ...
  let result = List.fold_left (fun v (lhs, rhs) -> v && checkCounterExample post_concept_hash1 post_concept_hash2 lhs rhs) true elRanCexList in
  if result then
