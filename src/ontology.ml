...
type t = {
  ...
  mutable roleDifferenceInformation : RoleDifference.t;
...
let create () =	{
  ...
  (* ========== role differences ========== *)
  roleDifferenceInformation = RoleDifference.create ();
  ...
  if not (S.mem opa ont.record_ObjectPropertyAxiom) then
    begin
      ...
      | C.SubObjectPropertyOf (ope_ch, ope) ->
          RoleDifference.processRoleInclusion ont.roleDifferenceInformation
                     (extractRoleNameFromExpressionList ope_ch)
                     (extractRoleNameFromExpression ope);
...
let get_RoleDifferenceInformation t = t.roleDifferenceInformation
...
let add_domain_range_concept_definitions roleNames ont occurrenceHash leftHandSideSet =
  let axiomList = StringSet.fold (fun l r -> let domainBinding = (RoleMapping.map_role_to_domain_binding r) in
                                             insert_in_occurrence_hash occurrenceHash domainBinding [];
                                             leftHandSideSet := StringSet.add !leftHandSideSet domainBinding;
                                 ...
                                 roleNames
  in
  List.iter (fun ax -> add_ClassExpressionAxiom ont ax) axiomList
