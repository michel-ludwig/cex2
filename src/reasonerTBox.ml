...
let init_is t core_pos core_neg =
  ...
  is_union_implied_neg t is core_neg;
  is_union_implied_pos t is core_pos;
  is
;;

let rt_assign_is t core_pos core_neg =
...
let rt_cons t core_pos core_neg =
  try HI.find t.rt_to_is (core_pos, core_neg)
  with Not_found ->
      let is = rt_assign_is t core_pos core_neg
      in is
;;

let empty_is t = rt_cons t ClassExpression.Set.empty ClassExpression.Set.empty

let init_fresh_edge t is c =
  incr total_children;
  ...
  let ed =
    { id = t.edge_count;
      ...
      is_child = rt_cons t (ClassExpression.Set.singleton c) ClassExpression.Set.empty;
...
(* [ch_is]                                                                 *)
let is_propagate_backward t ch_is concept_record =
  let iter_parents f =
  ...
  iter_parents ( fun _ ed_id_set (c_pos, c_neg) ->
          IS.iter (fun ed_id ->
                  ...
                  is_union_implied_pos t is c_pos;
                  is_union_implied_neg t is c_neg;
            ) ed_id_set
    );
;;
...
(* [ed]                                                                    *)
let ed_propagate_backward_c t ed concept_record =
  let iter_roles f =
...
let ed_extend_root t index ed c_pos c_neg =
  ...
  if not (ClassExpression.Set.is_empty c_pos && ClassExpression.Set.is_empty c_neg) then (
    ...
    iter_isibling_parent_edges (fun sed ->
            is_union_implied_neg t sed.is_parent c_neg;
            is_union_implied_pos t sed.is_parent c_pos;
    ...
    in iter_concepts (fun c ->
            ...
            ed_propagate_backward_c t ed c_cr
      ); (* iter_concepts *)
  )
;;
...
let is_propagate_forward t index pt_is concept_record =
  let iter_children f =
...
let process_is_implied_new_neg t index is c =
  let c_cr = I.find_concept_record index c in
...
let process_is t index is =
  match is.implied_new_pos with
  | c :: rest ->
      is.implied_new_pos <- rest;
      is.implied_new_neg <- c :: is.implied_new_neg;
      process_is_implied_new_pos t index is c;  (* tail recursive! *)
  | [] -> match is.implied_new_neg with
      | c :: rest ->
          is.implied_new_neg <- rest;
          if is_processed_is is then pop_stack_is t;
          process_is_implied_new_neg t index is c;  (* tail recursive! *)
      | [] -> failwith "[is] was already processed!"
;;

(* processes a new role [ar,ar_dir] of the edge [ed] *)
let process_edge_new_role t index ed ar ar_dir =
  let ar_rr = I.find_role_record index ar in
  ...
  iter_sibling_child_edges_r (fun sed ->
          if sed.id != ed.id then (
            ...
            ed_extend_root t index ed sed.is_child.core_pos sed.is_child.core_neg;
            ed_extend_root t index ed (ed_get_rt_child_new_pos sed) (ed_get_rt_child_new_neg sed);
  ...
  iter_sibling_child_edges_r (fun sed ->
      (* inserting new concepts *)
          is_union_implied_pos t ed.is_parent sed.is_child.core_pos;
          is_union_implied_neg t ed.is_parent sed.is_child.core_neg;
          is_union_implied_pos t ed.is_parent (ed_get_rt_child_new_pos sed);
          is_union_implied_neg t ed.is_parent (ed_get_rt_child_new_neg sed);
          (* inserting new roles *)
  ...
  ed_propagate_backward_r t ed ar_rr ar_dir;
  ed_propagate_forward_r t index ed ar_rr ar_dir;
  (* extend the sibling parents with new roles *)
  iter_isibling_parent_edges (fun sed ->
      (* inserting new concepts *)
          is_union_implied_pos t sed.is_parent ed.is_child.core_pos;
          is_union_implied_neg t sed.is_parent ed.is_child.core_neg;
          is_union_implied_pos t sed.is_parent (ed_get_rt_child_new_pos ed);
          is_union_implied_neg t sed.is_parent (ed_get_rt_child_new_neg ed);
          (* inserting new roles *)
          ed_union_roles t sed ed.rolesi ed.roles
    ) t index ed.is_parent ed.roles ed.rolesi;
;;

let process_edge_new_roles t index ed =
  
  match ed.roles_new with
  | ar :: rest ->
      ed.roles_new <- rest;
      if is_processed_ed_roles ed then pop_stack_ed_roles t;
      process_edge_new_role t index ed ar true;
  | [] ->
      match ed.rolesi_new with
      | ar :: rest ->
          ed.rolesi_new <- rest;
          if is_processed_ed_roles ed then pop_stack_ed_roles t;
          process_edge_new_role t index ed ar false;
...
let process_edge_root t index ed =
  ...
  let ch_is = try HI.find t.rt_to_is (new_core_pos, new_core_neg)
    with Not_found -> rt_assign_is t new_core_pos new_core_neg
  in
  ed.is_child <- ch_is;
  ...
  iter_sibling_child_edges_ed (fun sed ->
      (* inserting new concepts *)
          is_union_implied_pos t ed.is_parent sed.is_child.core_pos;
          is_union_implied_neg t ed.is_parent sed.is_child.core_neg;
          is_union_implied_pos t ed.is_parent sed.rt_child_new_pos;
          is_union_implied_neg t ed.is_parent sed.rt_child_new_neg;
          (* inserting new roles *)
          ed_union_roles t ed sed.rolesi sed.roles
...
let last_check = ref 0
let check_interval = 1

let check_integrity t index =
  let progress = PB.get_status () in
  if progress > !last_check + check_interval then (
    last_check := progress;
...
(*|  check_integrity t index;*)
  if !count_stack_is > !count_stack_ed_roles then
    ...
    | Some is ->
        ...
        process_is t index is;
        process_t t index (* tail recursive! *)
    ...
    | Some ed ->
        ...
        else process_edge_new_roles t index ed;
        process_t t index (* tail recursive! *)
    | None -> invalid_arg "process_t"
  else
    match t.current_ed_root with
    | Some ed ->
        ...
        if ed_is_deleted ed then
          pop_stack_ed_root t
        else process_edge_root t index ed;
        process_t t index (* tail recursive! *)
    | None -> () (* done! *)
;;
...
let saturate ont =
  ...
  let t = t_create (O.total_ClassIRI ont) ont in
...
let compute_post_for_concepts t =
  let postHash = Hashtbl.create 100
  in
    Ontology.iter_record_Class (fun c _ ->
                                  match c with Class.Constructor.IRI iri ->
                                    (match iri.data with IRI.Constructor.IRI cname ->
                                      begin
                                      let implied = find_implied t (O.cons_ClassExpression t.ontology (ClassExpression.Constructor.Class c)) in
                                      let handle_class_expression exp =
                                        match exp.data with
                                          ClassExpression.Constructor.Class d -> (match d with
                                                                                  Class.Constructor.IRI diri ->
                                                                                    (match diri.data with IRI.Constructor.IRI dname -> 
                                                                                      StringSet.update_hash_with_element postHash cname dname)
                                                                                  | _ -> ())
                                        | _ -> ()
                                      in
                                      ClassExpression.HSet.iter handle_class_expression implied;
                                      ()
                                      end)
                                | _ -> ())
                                t.ontology;
...
let compute_pre_post_for_concepts t =
  let preHash = Hashtbl.create (O.total_ClassIRI t.ontology) in
  let postHash = Hashtbl.create (O.total_ClassIRI t.ontology)
  in
    Ontology.iter_record_Class (fun c _ ->
                                  match c with Class.Constructor.IRI iri ->
                                    (match iri.data with IRI.Constructor.IRI cname ->
                                      ...
                                      let handle_class_expression exp =
                                        match exp.data with
                                          ClassExpression.Constructor.Class d -> (match d with
                                                                                  Class.Constructor.IRI diri ->
                                                                                    (match diri.data with IRI.Constructor.IRI dname -> 
                                                                                      StringSet.update_hash_with_element preHash dname cname;
                                                                                      StringSet.update_hash_with_element postHash cname dname)
                                                                                  | _ -> ())
                                        | _ -> ()
                                      in
...
let implied_concept_names t cname =
  let implied = find_implied t (O.concept_name_class_expression t.ontology cname) in
  let handle_class_expression exp l =
    match exp.data with
      ClassExpression.Constructor.Class d -> (match d with
                                                Class.Constructor.IRI iri -> (match iri.data with IRI.Constructor.IRI dname -> dname::l)
                                              | _ -> l)
    | _ -> l
  in
  ClassExpression.HSet.fold handle_class_expression implied []
