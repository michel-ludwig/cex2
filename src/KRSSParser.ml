...
let load_ontology input normalisationInformation =
  let lexbuf = Lexing.from_channel input in
  let leftHandSideSet = ref StringSet.empty in
  let (ont, occurrenceHash, normalisationInformation) =
    try
      Krss_parser.ontology Krss_lexer.token lexbuf leftHandSideSet normalisationInformation
  ...
  (ont, occurrenceHash, !leftHandSideSet, normalisationInformation)
;;
